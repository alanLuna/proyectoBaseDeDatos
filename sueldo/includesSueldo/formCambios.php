...
<div id="divModificar<?= $valor; ?>" class="container col-md-12 col-lg-12 col-xs-12 col-sm-12" style="display:none">  
    <select id="update<?= $valor?>" class="form-control">
        <option selected="true" disabled="disabled">Selecciona un sueldo: </option>
    </select>
    <form id="formModificar<?= $valor; ?>" class="form-horizontal" method="post" autocomplete="off" style="display:none;">
        <div class="form-group">
            ...
            <div class="col-md-9">
                <input class="form-control" type="text" name="nuevoSaldo<?= $valor ?>" id="nuevoSaldo<?= $valor ?>"
                placeholder="Ingrese el nuevo monto del saldo" required autocomplete="off">  
            </div>
        </div>
